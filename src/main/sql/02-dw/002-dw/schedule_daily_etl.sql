...
SET DATE_ADD(CURRENT_DATE(),-1) = DATE_ADD(${CURRENT_DATE()},-1);
SET ${hiveconf:max_date} = CAST('2050-01-01' AS DATE);
*/
-- 设置DB
use sales_dw;

--在hive-site.xml中设置max_date,cur_date,pre_date
...
-- SELECT last_load, ${CURRENT_DATE()} current_load FROM sales_rds.cdc_time;
INSERT overwrite TABLE sales_rds.cdc_time
SELECT ${hiveconf:max_date}, ${hiveconf:pre_date} current_load ;--FROM sales_rds.cdc_time;
